...
def _get_box_class_field(eval_boxes: EvalBoxes) -> str:
    ...
    """
    if len(eval_boxes.boxes) == 0:
        print("Warning: No boxes in eval_boxes!")
        print(f"eval_boxes: {eval_boxes}")
        return None  # 或者根据需要返回一个默认值
    # 必须保证用于eval的3dbox数量大于0
...
class FreeWorldEvalCustom(NuScenesEval):
    """
...
if __name__ == "__main__":
    ...
    freew_eval = FreeWorldEvalCustom(nusc_, config=cfg_, result_path=result_path_, eval_set=eval_set_,
                                    output_dir=output_dir_, verbose=verbose_)
    for vis in ['1', '2', '3', '4']:
        freew_eval.update_gt(type_='vis', visibility=vis)
        print(f'================ {vis} ===============')
        freew_eval.main(plot_examples=plot_examples_, render_curves=render_curves_)
    #for index in range(1, 41):
    #    freew_eval.update_gt(type_='ord', index=index)
    #
