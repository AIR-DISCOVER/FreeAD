...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def prepare_train_data(self, index):
        ...
        assert len(prev_indexs_list) > 0, f"prev_indexs_list is empty for index {index}"

        input_dict = self.get_data_info(index)
        assert input_dict is not None, f"get_data_info returned None for index {index}"

        # Check key fields in input_dict
        assert 'frame_idx' in input_dict, f"Missing 'frame_idx' in input_dict for index {index}"
        assert 'scene_token' in input_dict, f"Missing 'scene_token' in input_dict for index {index}"
        ...
        assert example is not None, f"pipeline returned None for index {index}"
        # Print example to debug
        print(f"Example for index {index}: {example}")

        example = self.vectormap_pipeline(example, input_dict)
        assert example is not None, f"vectormap_pipeline returned None for index {index}"
        
        # Print example to debug
        #print(f"Example for index {index}: {example}")

        if self.filter_empty_gt:
            gt_labels_3d_valid = example.get('gt_labels_3d') is not None and (example['gt_labels_3d']._data != -1).any()
            map_gt_labels_3d_valid = example.get('map_gt_labels_3d') is not None and (example['map_gt_labels_3d']._data != -1).any()
            print(f"Index {index}: gt_labels_3d_valid={gt_labels_3d_valid}, map_gt_labels_3d_valid={map_gt_labels_3d_valid}")
        ...
        for i in prev_indexs_list:
            ...
            assert input_dict is not None, f"get_data_info failed for previous index {i}"

            # Check the consistency of frame_idx and scene_token
            assert input_dict['frame_idx'] < frame_idx, f"Frame idx mismatch for prev index {i}, current frame {frame_idx}"
            assert input_dict['scene_token'] == scene_token, f"Scene token mismatch for prev index {i}, current scene_token {scene_token}"
            ...
            if self.filter_empty_gt and \
                ...
                raise ValueError(f"filter_empty_gt triggered for previous index {i}")
            
            frame_idx = input_dict['frame_idx']
            data_queue.insert(0, copy.deepcopy(example))

        # Final validation of data_queue before processing
        assert len(data_queue) > 0, f"Data queue is empty after processing index {index}"
    ...
    # Attention: this may cause loop when datasets is None
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if data is None:
                attempts += 1
                print(f"Attempt {attempts} failed for idx {idx}. Trying another index...")
                
                idx = self._rand_another(idx)
                # 打印更换的索引
                print(f"Switched to idx {idx}")
                
                # 如果尝试次数过多，可以退出循环，避免死循环
                if attempts > 10:
                    raise ValueError(f"Unable to get valid data after 10 attempts at idx {idx}")
                
                continue
            else:
                print(f"Successfully loaded data for idx {idx}")
                return data
    ...
    def evaluate(self,
        ...
        for cls in motion_cls_names:
            ...
            if all_metric_dict['gt_'+cls] != 0:  # 检查除数是否为零
                result_dict['EPA_'+cls] = (all_metric_dict['hit_'+cls] - alpha * all_metric_dict['fp_'+cls]) / all_metric_dict['gt_'+cls]
            else:
                result_dict['EPA_'+cls] = 0  # 或者可以设置为一个默认值，例如0

            if all_metric_dict['cnt_ade_'+cls] != 0:
                result_dict['ADE_'+cls] = all_metric_dict['ADE_'+cls] / all_metric_dict['cnt_ade_'+cls]
            else:
                result_dict['ADE_'+cls] = 0  # 或者其他合理的默认值

            if all_metric_dict['cnt_fde_'+cls] != 0:
                result_dict['FDE_'+cls] = all_metric_dict['FDE_'+cls] / all_metric_dict['cnt_fde_'+cls]
                result_dict['MR_'+cls] = all_metric_dict['MR_'+cls] / all_metric_dict['cnt_fde_'+cls]
            else:
                result_dict['FDE_'+cls] = 0
                result_dict['MR_'+cls] = 0
            
            # result_dict['EPA_'+cls] = 0.1
            # result_dict['ADE_'+cls] = 0.1
            # result_dict['FDE_'+cls] = 0.1
            # result_dict['MR_'+cls] = 0.1
