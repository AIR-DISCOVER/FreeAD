...
# + Methods operating on FreeWorldMap and NuScenes by Holger Caesar, 2019.
...
class FreeWorldMap:
    """
    FreeWorldMap database class for querying and retrieving information from the semantic maps.
    ...
    def __init__(self,
        ...
        """
        assert map_dim[0] > 0 and map_dim[1] > 0, "Error: map_dim values must be positive (width > 0, height > 0)."
        ...
        self.layer_names = self.geometric_layers + self.lookup_polygon_layers + self.non_geometric_line_layers  # in FreeWorld, we only define road_divider type
        ...
        self.explorer = FreeWorldMapExplorer(self)
...
class FreeWorldMapExplorer:
    ...
    def get_map_mask(self,
        ...
        :return: Stacked numpy array of size [c x h x w] with c channels and the same width/height as the canvas.
        """
        if self.map_dims[0] <= 0 or self.map_dims[1] <= 0:
            raise Exception('Error: Invalid map dims! Both dimensions must be greater than zero.')
  
        # If None, return the entire map.
        if patch_box is None:
            patch_box = [self.map_dims[0] / 2, self.map_dims[1] / 2, self.map_dims[1], self.map_dims[0]]
    ...
    def render_map_in_image(self,
        ...
        # Check that FreeWorldMap was loaded for the correct location.
        sample_record = nusc.get('sample', sample_token)
        scene_record = nusc.get('scene', sample_record['scene_token'])
        log_record = nusc.get('log', scene_record['log_token'])
        log_location = log_record['location']
        assert self.map_api.map_name == log_location, \
            'Error: FreeWorldMap loaded for location %s, should be %s!' % (self.map_api.map_name, log_location)
            ...
            for token in records_in_patch[layer_name]:
                ...
                for polygon_token in polygon_tokens:
                    ...
                    if render_behind_cam:
                        ...
                        points = FreeWorldMapExplorer._clip_points_behind_camera(points, near_plane)
                    elif np.any(behind):
    ...
    def _render_polygon_layer(self, ax: Axes, layer_name: str, alpha: float, tokens: List[str] = None) -> None:
        ...
        '''
            In FreeWorld, We don't have drivable_area
        '''
        # if layer_name == 'drivable_area':
        #     for record in records:
        #         polygons = [self.map_api.extract_polygon(polygon_token) for polygon_token in record['polygon_tokens']]
