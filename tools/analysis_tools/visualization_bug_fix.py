...
from FreeWorld.utils.ros_coord_to_bev_coord import ROSCoord2NuscenesBEVCoord
from tools.analysis_tools.serialize_bev_results import *
...
if __name__ == '__main__':
    ...
    #bevformer_results = ROSCoord2NuscenesBEVCoord(bevformer_results) # transform ROS coord to NuscenesBEV coord
    sample_token_list = list(bevformer_results['results'].keys())

 
    # # 将整个 bevformer_results 对象递归转换为原生 Python 类型
    # # 使用自定义的 JSONEncoder 进行序列化
    output_file_path = 'bevformer_results.json'  # 输出文件路径
    with open(output_file_path, 'w') as f:
        json.dump(bevformer_results, f, cls=NumpyEncoder, indent=4)
    

    nusc = NuScenes(version='v1.0-trainval', dataroot='./data/FreeWorld_mini', verbose=True)
    ...
    for id in tqdm(range(len(sample_token_list))):
        ...
        for cam in cams:
            ...
            if sensor_modality in ['lidar', 'radar']:
                ...
                if cam == 'CAM_FRONT':
                    # 获取LIDAR的相关记录信息
                    lidar_sd_record =  nusc.get('sample_data', sample['data']['LIDAR_TOP'])  # 获取LIDAR样本数据记录
                    lidar_cs_record = nusc.get('calibrated_sensor', lidar_sd_record['calibrated_sensor_token'])  # 获取LIDAR的校准信息
                    lidar_pose_record = nusc.get('ego_pose', lidar_sd_record['ego_pose_token'])  # 获取LIDAR的位姿信息

                    # 获取规划轨迹的命令（规划目标的执行）
                    plan_cmd = np.argmax(bevformer_results['plan_results'][sample_token][1][0, 0, 0])  # 从预测结果中选择最佳的规划命令
                    plan_traj = bevformer_results['plan_results'][sample_token][0][plan_cmd]  # 获取选择的规划轨迹
                    plan_traj[abs(plan_traj) < 0.01] = 0.0  # 将接近零的数值设置为零
                    plan_traj = plan_traj.cumsum(axis=0)  # 对轨迹进行累积求和，得到逐步的轨迹坐标

                    # 将规划轨迹转换为LiDAR坐标系下的 [x, y, z, w] 四元数表示，w 固定为 1
                    plan_traj = np.concatenate((
                        plan_traj[:, [0]],  # x 坐标
                        plan_traj[:, [1]],  # y 坐标
                        -1.0 * np.ones((plan_traj.shape[0], 1)),  # z 坐标（设为 -1，表示接近地面）
                        np.ones((plan_traj.shape[0], 1)),  # w 坐标（设为 1）
                    ), axis=1)

                    # 将起点添加到轨迹的开始，确保轨迹起点在 LiDAR 坐标系下是原点
                    plan_traj = np.concatenate((np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0)
                    plan_traj[0, 0] = 0.3  # 设置轨迹起点 x 坐标
                    plan_traj[0, 2] = -1.0  # 设置轨迹起点 z 坐标
                    plan_traj[0, 3] = 1.0  # 设置轨迹起点 w 坐标

                    # 获取 LiDAR 到传感器的旋转矩阵和位移向量
                    l2e_r = lidar_cs_record['rotation']  # 获取 LIDAR 到ego的旋转矩阵
                    l2e_t = lidar_cs_record['translation']  # 获取 LIDAR 到ego的位移向量
                    e2g_r = lidar_pose_record['rotation']  # 获取 ego 车体到全球坐标的旋转矩阵
                    e2g_t = lidar_pose_record['translation']  # 获取 ego 车体到全球坐标的位移向量

                    # 计算旋转矩阵
                    l2e_r_mat = Quaternion(l2e_r).rotation_matrix  # 将旋转四元数转换为旋转矩阵
                    e2g_r_mat = Quaternion(e2g_r).rotation_matrix  # 将旋转四元数转换为旋转矩阵

                    # 计算 LiDAR 到相机坐标系的转换矩阵
                    s2l_r, s2l_t = obtain_sensor2top(nusc, sample_data_token, l2e_t, l2e_r_mat, e2g_t, e2g_r_mat, cam)

                    # 计算从 LiDAR 到相机坐标系的旋转和位移
                    lidar2cam_r = np.linalg.inv(s2l_r)  # 计算旋转矩阵的逆矩阵
                    lidar2cam_t = s2l_t @ lidar2cam_r.T  # 计算位移向量

                    # 组合旋转矩阵和位移向量，得到 LiDAR 到相机的 4x4 变换矩阵
                    lidar2cam_rt = np.eye(4)  # 初始化 4x4 单位矩阵
                    lidar2cam_rt[:3, :3] = lidar2cam_r.T  # 设置旋转矩阵
                    lidar2cam_rt[3, :3] = -lidar2cam_t  # 设置位移向量

                    # 设置相机的内参矩阵
                    viewpad = np.eye(4)  # 初始化 4x4 单位矩阵
                    viewpad[:camera_intrinsic.shape[0], :camera_intrinsic.shape[1]] = camera_intrinsic  # 填充相机内参

                    # 将相机内参与 LiDAR 到相机的变换矩阵结合，得到最终的 LiDAR 到图像的变换矩阵
                    lidar2img_rt = (viewpad @ lidar2cam_rt.T)

                    # 将规划轨迹从 LiDAR 坐标系映射到图像坐标系
                    plan_traj = lidar2img_rt @ plan_traj.T
                    plan_traj = plan_traj[0:2, ...] / np.maximum(
                        plan_traj[2:3, ...], np.ones_like(plan_traj[2:3, ...]) * 1e-5)  # 进行透视除法，得到归一化的图像坐标
                    plan_traj = plan_traj.T  # 转置回原始格式

                    # 将轨迹分段，每两个点之间插入更多的点进行平滑处理
                    plan_traj = np.stack((plan_traj[:-1], plan_traj[1:]), axis=1)

                    # 计算规划轨迹的线段，用于可视化
                    plan_vecs = None
                    for i in range(plan_traj.shape[0]):
                        plan_vec_i = plan_traj[i]  # 获取每一段的起点和终点
                        x_linspace = np.linspace(plan_vec_i[0, 0], plan_vec_i[1, 0], 51)  # 在 x 轴上生成 51 个点
                        y_linspace = np.linspace(plan_vec_i[0, 1], plan_vec_i[1, 1], 51)  # 在 y 轴上生成 51 个点
                        xy = np.stack((x_linspace, y_linspace), axis=1)  # 将 x 和 y 坐标合并
                        xy = np.stack((xy[:-1], xy[1:]), axis=1)  # 生成连续的轨迹线段
                    ...
                    # 生成可视化的颜色映射
                    cmap = 'winter'  # 设置颜色映射为冬季色调
                    y = np.sin(np.linspace(1/2 * np.pi, 3/2 * np.pi, 301))  # 生成一个正弦波
                    colors = color_map(y[:-1], cmap)  # 为正弦波的每个值选择一种颜色

                    # 创建轨迹线段的可视化对象
                    line_segments = LineCollection(plan_vecs, colors=colors, linewidths=2, linestyles='solid', cmap=cmap)
                    ax.add_collection(line_segments)  # 将轨迹线段添加到图形中进行可视化
