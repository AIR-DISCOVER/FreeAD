...
from .FreeAD_custom_FreeWorld_eval import FreeWorldEvalCustom
...
from FreeWorld.map_expansion.map_api import FreeWorldMap, FreeWorldMapExplorer
...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
        ...
        self.nusc_maps = {}
        self.map_explorer = {}
        for loc, map_dim in self.MAPS.items():  # 直接解包 loc 和 map_dim
            self.nusc_maps[loc] = FreeWorldMap(dataroot=self.data_root, map_name=loc, map_dim=map_dim)
            self.map_explorer[loc] = FreeWorldMapExplorer(self.nusc_maps[loc])
...
class FreeWorldDataset(NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                if not isinstance(cam_info['cam_intrinsic'], np.ndarray):
                    cam_info['cam_intrinsic'] = np.array(cam_info['cam_intrinsic'])
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if retries >= max_retries:
                retries = 0
                print("Warning: 10 times can't find valid data while training")
            if data is None:
    ...
    def _evaluate_single(self,
        ...
        self.freew_eval = FreeWorldEvalCustom(
            self.nusc,
        ...
        self.freew_eval.main(plot_examples=0, render_curves=False)
        # record metrics
    ...
    def evaluate(self,
        ...
        for cls in motion_cls_names:
            ...
            # 计算 EPA (Entity Precision Accuracy) 指标
            # 'hit_'+cls: 该类别的命中数（模型预测的正确数量）
            # alpha: 一个权重因子，用于调节假阳性对最终结果的影响
            # 'fp_'+cls: 该类别的假阳性数量（模型错误预测为正例的数量）
            # 'gt_'+cls: 该类别的真实目标数（ground truth, 实际目标的数量）
            
            # Check for division by zero for EPA
            if all_metric_dict['gt_'+cls] == 0:
                print(f"Warning: Division by zero encountered for EPA calculation in class '{cls}' (gt_{cls} = 0).")
                result_dict['EPA_'+cls] = float('nan')  # Set default value to NaN or any other suitable value (e.g., 0.0)
            else:
                result_dict['EPA_'+cls] = (all_metric_dict['hit_'+cls] - \
                    alpha * all_metric_dict['fp_'+cls]) / all_metric_dict['gt_'+cls]
            
            # 计算 ADE (Average Displacement Error) 指标
            # 'ADE_'+cls: 该类别的平均位移误差（模型预测轨迹与真实轨迹之间的平均距离）
            # 'cnt_ade_'+cls: 该类别的 ADE 计算时涉及的样本数量
            if all_metric_dict['cnt_ade_'+cls] == 0:
                print(f"Warning: Division by zero encountered for ADE calculation in class '{cls}' (cnt_ade_{cls} = 0).")
                result_dict['ADE_'+cls] = float('nan')  # Set default value to NaN or another value
            else:
                result_dict['ADE_'+cls] = all_metric_dict['ADE_'+cls] / all_metric_dict['cnt_ade_'+cls]
            
            # 计算 FDE (Final Displacement Error) 指标
            # 'FDE_'+cls: 该类别的最终位移误差（模型预测轨迹的最后点与真实轨迹的最后点之间的距离）
            # 'cnt_fde_'+cls: 该类别的 FDE 计算时涉及的样本数量
            if all_metric_dict['cnt_fde_'+cls] == 0:
                print(f"Warning: Division by zero encountered for FDE calculation in class '{cls}' (cnt_fde_{cls} = 0).")
                result_dict['FDE_'+cls] = float('nan')  # Set default value to NaN or another value
            else:
                result_dict['FDE_'+cls] = all_metric_dict['FDE_'+cls] / all_metric_dict['cnt_fde_'+cls]
            
            # 计算 MR (Miss Rate) 指标
            # 'MR_'+cls: 该类别的错过率（漏检的目标数量 / 总目标数量）
            # 'cnt_fde_'+cls: 该类别的 MR 计算时涉及的样本数量
            if all_metric_dict['cnt_fde_'+cls] == 0:
                print(f"Warning: Division by zero encountered for MR calculation in class '{cls}' (cnt_fde_{cls} = 0).")
                result_dict['MR_'+cls] = float('nan')  # Set default value to NaN or another value
            else:
                result_dict['MR_'+cls] = all_metric_dict['MR_'+cls] / all_metric_dict['cnt_fde_'+cls]
